...
function calcTwoPairs(diceCount){
  let p1 = calcIdentical(diceCount,2);
  if (p1 > 0) {
    for(let dice=(p1/2)-1;dice>=minDice;dice--)
    if(diceCount[dice]>=2)
      return dice*2 + p1;
  }
  return 0;
...
//computes the score for chance
function calcChance(diceCount){
  let sum = 0;
  diceCount.forEach((die, index) => {
    sum += index * die;
  });
  return sum;
}
...
function calcYatzy(diceCount){
  return (calcIdentical(diceCount, 5) > 0) ? 50 : 0;
}
