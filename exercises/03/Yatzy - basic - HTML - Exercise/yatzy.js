...
function printScoresHTML(scoreTable){
  ...
  let res=`To get help on playing the game, please consult the <a href="resources/help.html" title="Help Page for Yatzy">help page</a>, and general description of the <a href="https://da.wikipedia.org/wiki/Yatzy" title="Wikipedia entry on Yatzy">Yatzy game</a>`
  res+=`<table id="scoretable"> \n`;
  ...
  for(let round=0;round<scoreTable.length;round++) {
    ...
    if(!isSpecialRound(round)){
      ...
      for(let d=0;d<scoreTable[round].diceRoll.length;d++){
         ...
         res+=`<img src="resources/${imgName}-dice.png" title="dice ${d}" alt="dice ${d}" width="20" height="20" >`;
      }
      res+="</span>";
    }
    res+="</td>"
